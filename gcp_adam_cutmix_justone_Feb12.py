...
@dataclass
class CFG:
    description: str = "gcp adam cutmix just one, higher lr"
...
if __name__ == "__main__":
    ...
    for fold, (train_idxs, val_idxs) in enumerate(folds):
        ...
        train_dset = CutMix(pre_cutmix_train_dset, num_class=5, beta=cfg.cutmix_beta, prob=cfg.cutmix_prob)
    ...
    #     leaf_model.scheduler = get_one_cycle(leaf_model.optimizer, start_lr, max_lr, final_lr, num_epochs, steps_per_epoch=len(train_dataloader))
        ...
        trainer.train_epochs(list(range(2, 2 + cos_epochs)))
