...
if __name__ == "__main__":
    ...
    @dataclass
    class CFG:
        description: str = "b4 446 final 10 theirs whole"
        model_file: str = "final10-theirs-whole"
        num_classes: int = 5
        img_size: int = 446
    ...
    dset_2020 = DistillationDataSet(dset_2020, "./data/theirs/soft_targets_2020.csv", soft_ratio=cfg.soft_ratio, soft_start_col=1)
    ...
    train_dset = train_dset = UnionDataSet(dset_2020, dset_2019, transform=train_transforms)
    ...
    model_prefix = f"{cfg.model_file}_{datetime.now().strftime('%b%d_%H-%M-%S')}"
