...
if __name__ == "__main__":
    ...
    min_min_lr = 1e-6
    max_min_lr = 1e-3
    weight_decay_list = [0.0, 1e-4, 1e-5, 1e-6]
    ...
    for num_run in range(num_runs):
        min_lr_exp_range = (np.log2(min_min_lr), np.log2(max_min_lr))
        max_lr_exp_range = (np.log2(min_max_lr), np.log2(max_max_lr))
        min_lr_exp = random.uniform(*min_lr_exp_range)
        max_lr_exp = random.uniform(*max_lr_exp_range)
        min_lr = np.exp2(min_lr_exp)
        max_lr = np.exp2(max_lr_exp)
        weight_decay = random.choice(weight_decay_list)
        model_prefix = f"{arch}_1cycle-fixd_{datetime.now().strftime('%b%d_%H-%M-%S')}"
        ...
        neptune.create_experiment(name=model_prefix, params=params_dict, upload_source_files=['*.py', 'leaf/*.py', 'environment.yml'],
                                  description="Runs with 1cycle learning rate policy, exploring different min/max values for learning rate and different weight decays.")
        neptune.log_text("params", f"{json.dumps(params_dict)}")
