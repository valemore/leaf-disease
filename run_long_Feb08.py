...
if __name__ == "__main__":
    ...
    for fold, (train_idxs, val_idxs) in enumerate(folds):
        ...
        scheduler = CosineAnnealingLR(optimizer, T_max=7 * len(train_dataloader), eta_min=min_lr)
        ...
        for epoch in range(1, num_epochs+1):
            ...
            if epoch >= 8 and epoch < 15:
                train_one_epoch(leaf_model, train_dataloader, log_steps=log_steps, epoch_name=f"{epoch_name}", steps_offset=steps_offset, neptune=neptune, grad_norm=grad_norm)
                steps_offset += len(train_dataloader)
            ...
            if epoch == 15:
                ...
                cutmix_num_mix = 2 + num_mix_cycle
                train_dset = CutMix(pre_cutmix_train_dset, num_class=5, beta=1.0, prob=cutmix_prob, num_mix=cutmix_num_mix, transform=post_cutmix_transforms)
                train_dataloader = LeafDataLoader(train_dset, batch_size=batch_size, shuffle=True, num_workers=num_workers)
            ...
            if epoch >= 15:
                num_mix_cycle = (num_mix_cycle + 1) % 3
                cutmix_num_mix = 2 + num_mix_cycle
                train_dset = CutMix(pre_cutmix_train_dset, num_class=5, beta=1.0, prob=cutmix_prob, num_mix=cutmix_num_mix, transform=post_cutmix_transforms)
                train_dataloader = LeafDataLoader(train_dset, batch_size=batch_size, shuffle=True, num_workers=num_workers)
