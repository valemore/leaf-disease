...
if __name__ == "__main__":
    ...
    for fold, (train_idxs, val_idxs) in enumerate(folds):
        ...
        sampler = BalanceClassSampler(train_dset.labels, mode="downsampling")

        train_dataloader = LeafDataLoader(train_dset, batch_size=batch_size, shuffle=False, sampler=sampler, num_workers=num_workers)
        ...
        # scheduler = CosineAnnealingLR(optimizer, T_max=num_epochs * len(train_dataloader), eta_min=min_lr)
        # scheduler = LambdaLR(optimizer, lr_lambda=lambda step: decay_factor ** (step if on_gcp else int(step / 3)))
        scheduler = OneCycleLR(optimizer, epochs=num_epochs, steps_per_epoch=363, max_lr=max_lr, div_factor=div_factor)
        leaf_model.update_optimizer_scheduler(optimizer, scheduler)

        neptune.init(project_qualified_name='vmorelli/leaf')
        params_dict = {
            param: eval(param) for param in ["cfg", "train_transforms", "post_cutmix_transforms", "val_transforms", "batch_size", "num_epochs", "max_lr", "min_lr", "weight_decay", "optimizer", "scheduler", "grad_norm"]
        }
        neptune_tags = []
